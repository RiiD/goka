...
type emitter func(topic string, key []byte, value []byte) *kafka.Promise
...
func (ctx *cbContext) Emit(topic Stream, key interface{}, value interface{}) {
	...
	if keyCodec == nil {
		ctx.Fail(fmt.Errorf("no key codec for topic %s", topic))
	}
...
func (ctx *cbContext) Loopback(key interface{}, value interface{}) {
	...
	if err != nil {
		ctx.Fail(fmt.Errorf("error encoding message value for key %s: %v", key, err))
	}

	keyBytes, err := l.KeyCodec().Encode(key)
	if err != nil {
		ctx.Fail(fmt.Errorf("error encoding message key for key %s: %v", key, err))
	}
...
func (ctx *cbContext) Key() interface{} {
	...
	if err != nil {
		ctx.Fail(fmt.Errorf("error decoding key %v", ctx.msg.Key))
	}
